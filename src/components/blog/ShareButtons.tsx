...
import { Share2, Linkedin, Mail, Link as LinkIcon, Check } from "lucide-react";
...
export default function ShareButtons({ title, slug, excerpt }: ShareButtonsProps) {
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap">
      <span className="flex items-center gap-1.5 text-sm font-medium text-slate-body">
        <Share2 size={14} />
        Teilen:
      </span>
      {shareLinks.map((link) => (
        <a
          ...
          className={`inline-flex items-center justify-center w-9 h-9 rounded-full border border-gray-200 text-gray-500 transition-all duration-200 ${link.color}`}
        >
          {link.icon}
        </a>
      ))}
